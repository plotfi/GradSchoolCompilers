...
VOID write_inst(ADDRINT iaddr)
{
  ...
  if (gzwrite(stream[0], trace_op,sizeof(Trace_op)) != sizeof(Trace_op))
    cout << "Error when writting instruction " << inst_count[0] << endl;
  inst_count[0] = inst_count[0] + 1;
}
...
VOID dprint_inst(ADDRINT iaddr, string *disassemble_info) {
  Trace_op *trace_op = inst_storage[iaddr]; 
  std::cout << "*****begin of the data structure **** " << dec << inst_count[0] << endl;
